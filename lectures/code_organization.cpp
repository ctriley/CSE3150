...
void Student::readStudent() {
    ...
    cout << "Good morning " << fName
        << "! Please enter your midterm adn final: "
        << "(hit reutrn after the two numbers)";
    cin >> mt >> final;
    cout << "Now enter your homeworks --hit EOF (CNTL-D) at the end--: ";
    double x;
    while(cin >> x) {
        hmwks.push_back(x);
    }
...
double Student::median() const {
    auto sz = hmwks.size();
    if(sz % 2 == 0) {
        return (hmwks[sz - 1] + hmwks[sz/2])/2;
    } else {
        return hmwks[sz/2];
    }
}
